...
const helpContent: Record<string, { title: string; description: string }> = {
  ...
  faq: {
    ...
    description: 'Helpful answers about choosing products, Korean beauty rituals, and JISOO shopping details.',
  },
  shipping: {
...
function HelpPlaceholder({ title, description }: { title: string; description: string }) {
  return (
    <main className="min-h-screen bg-warm-ivory">
      <Header />
      <section className="pt-32 pb-24 lg:pt-40 lg:pb-32">
        <div className="max-w-3xl mx-auto px-4 lg:px-6 text-center">
          <p className="mb-4 text-xs uppercase tracking-[0.26em] text-rose-mauve">JISOO Experience</p>
          <h1 className="mb-5 font-serif text-4xl text-charcoal lg:text-5xl">{title}</h1>
          <p className="text-lg leading-relaxed text-muted-foreground">{description}</p>
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            <Link href="/help/contact" className="rounded-full bg-gradient-to-r from-rose-mauve to-[#d3af84] px-6 py-3 text-sm font-medium text-white transition-all hover:brightness-105">
              Contact Support
            </Link>
            <Link href="/shop" className="rounded-full border border-blush-pink px-6 py-3 text-sm font-medium text-charcoal transition-all hover:border-rose-mauve">
              Continue Shopping
            </Link>
          </div>
        </div>
  ...
  return (
    <main className="min-h-screen bg-warm-ivory">
      ...
      <section className="relative overflow-hidden pt-32 pb-24 lg:pt-40 lg:pb-32">
        <div aria-hidden="true" className="absolute inset-x-0 top-0 h-72 bg-gradient-to-b from-nude-beige/70 to-transparent" />
        <div className="relative mx-auto max-w-6xl px-4 lg:px-6">
          <div className="mx-auto max-w-3xl text-center">
            <p className="mb-4 text-xs font-semibold uppercase tracking-[0.26em] text-rose-mauve">JISOO Concierge</p>
            <h1 className="font-serif text-4xl font-medium text-charcoal lg:text-5xl">Frequently Asked Questions</h1>
            <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-charcoal/65">
              Simple answers for product choice, JISOO rituals, and shopping details.
            </p>
          </div>
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-3">
            {faqCards.map((card) => (
              ...
              >
                <div aria-hidden="true" className="absolute -right-12 -top-12 h-32 w-32 rounded-full bg-white/35 blur-2xl" />
                ...
                <h2 className="relative mt-7 font-serif text-2xl font-semibold leading-tight text-charcoal">{card.title}</h2>
                <p className="relative mt-4 text-sm leading-7 text-charcoal/65">{card.body}</p>
              </article>
            ))}
          </div>
          ...
          <div className="mx-auto mt-16 max-w-3xl rounded-[2rem] border border-blush-pink/30 bg-nude-beige/45 px-6 py-8 text-center shadow-[0_18px_60px_rgba(79,54,60,0.06)]">
            <h2 className="font-serif text-2xl font-medium text-charcoal">Need a more personal answer?</h2>
            <p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-charcoal/65">
              Contact the JISOO concierge for product guidance, order questions, or region-specific availability.
            </p>
            <div className="mt-7 flex flex-wrap justify-center gap-3">
              <Link href="/help/contact" className="rounded-full bg-gradient-to-r from-rose-mauve to-[#d3af84] px-7 py-3 text-sm font-medium text-white transition-all hover:brightness-105">
                Contact Support
              </Link>
              <Link href="/shop" className="rounded-full border border-blush-pink px-7 py-3 text-sm font-medium text-charcoal transition-all hover:border-rose-mauve">
                Continue Shopping
              </Link>
            </div>
          </div>
...
export default async function HelpPage({ params }: { params: Promise<{ slug?: string[] }> }) {
  ...
  return <HelpPlaceholder title={content.title} description={content.description} />
}
