...
export default function LegalPage() {
  ...
  const { document, ui } = getLegalDocument(params.slug ?? 'privacy', locale)
    ...
    <main className="min-h-screen bg-warm-ivory text-charcoal">
      ...
      <section className="pt-32 pb-20 lg:pt-40">
        <div className="mx-auto max-w-5xl px-4 lg:px-6">
          ...
          <div className="mt-8 grid gap-4 rounded-3xl border border-[#cfae83]/24 bg-[var(--card)] p-5 shadow-editorial lg:grid-cols-[0.7fr_1.3fr]">
            <div>
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-charcoal/54">{ui.region}</p>
              <p className="mt-1 leading-7 text-charcoal/72">{document.regionNotice}</p>
            </div>
          </div>
          <p className="mt-4 rounded-2xl border border-[#cfae83]/20 bg-white/22 p-4 text-sm leading-6 text-charcoal/64">{ui.disclaimer}</p>
              ...
              <article key={section.title} className="rounded-3xl border border-[#cfae83]/22 bg-white/24 p-6 shadow-[0_16px_36px_rgba(44,37,40,0.07)] backdrop-blur-xl">
                ...
                <div className="mt-4 space-y-3">
                  {section.body.map((paragraph) => (
                    <p key={paragraph} className="leading-7 text-charcoal/72">{paragraph}</p>
                  ))}
                </div>
              </article>
            ))}
