...
export function Header({
  ...
  const menuLabel = locale === 'ar' ? 'القائمة' : locale === 'fr' ? 'Menu' : locale === 'de' ? 'Menü' : locale === 'ko' ? '메뉴' : locale === 'tr' ? 'Menü' : 'Menu'
  const designSwitchInstruction = locale === 'ar' ? 'اضغطي للتبديل' : locale === 'fr' ? 'Cliquez pour changer' : locale === 'de' ? 'Zum Wechseln klicken' : locale === 'ko' ? '전환하려면 클릭하세요' : locale === 'tr' ? 'Değiştirmek için tıklayın' : 'Click to switch'
  const designModeTitle = locale === 'ar' ? `وضع التصميم: ${siteMode}` : locale === 'fr' ? `Mode design : ${siteMode}` : locale === 'de' ? `Designmodus: ${siteMode}` : locale === 'ko' ? `디자인 모드: ${siteMode}` : locale === 'tr' ? `Tasarım modu: ${siteMode}` : `Design mode: ${siteMode}`
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--warm-ivory', elegant ? '#e8e1d4' : '#f4e5cc')
    root.style.setProperty('--background', elegant ? '#e8e1d4' : '#f4e5cc')
          ...
          <div className="relative z-10 flex items-center gap-2">
            <div className="hidden items-center gap-2 lg:flex">
              <button
                ...
                aria-label={`${designModeTitle}. ${designSwitchInstruction}.`}
