  ...
  return (
    <AtmosphereSection atmosphere="ivory" className="relative overflow-hidden pt-28 pb-20 lg:pt-32 lg:pb-24">
      ...
      <div className="relative mx-auto max-w-7xl px-4 lg:px-6">
        <div className="mb-8 rounded-3xl border border-rose-mauve/20 bg-white/75 p-4 shadow-[0_18px_44px_rgba(201,153,166,0.15)] backdrop-blur-sm lg:p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-[11px] uppercase tracking-[0.28em] text-rose-mauve">Hero Showcase</p>
              <h2 className="mt-2 font-serif text-2xl text-charcoal lg:text-3xl">10 Premium Direction Concepts</h2>
              <p className="mt-1 text-sm text-charcoal/65">{active.subtitle}</p>
            </div>
            <span className="rounded-full border border-rose-mauve/25 bg-white/80 px-4 py-1.5 text-xs uppercase tracking-[0.18em] text-charcoal/70">
              {active.mood}
            </span>
          </div>
            ...
            {heroConcepts.map((concept, index) => (
              <button
                ...
                className={cn(
                  ...
                  activeId === concept.id
                    ? 'border-transparent bg-gradient-to-r from-rose-mauve to-[#d8b894] text-white shadow-md'
                    : 'border-rose-mauve/20 bg-white/80 text-charcoal/75 hover:border-rose-mauve/40 hover:bg-white'
                )}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {active.id === 'editorial-rose' && <EditorialRoseHero locale={locale} />}
            {active.id === 'modern-minimal' && <ModernMinimalHero locale={locale} />}
            {active.id === 'product-spotlight' && <ProductSpotlightHero locale={locale} />}
            {active.id === 'soft-campaign' && <SoftCampaignHero locale={locale} />}
            {active.id === 'ivory-lab' && <IvoryLabHero locale={locale} />}
            {active.id === 'film-frame' && <FilmFrameHero locale={locale} />}
            {active.id === 'image-led' && <ImageLedHero locale={locale} />}
            {active.id === 'commerce-split' && <CommerceSplitHero locale={locale} />}
            {active.id === 'mist-glass' && <MistGlassHero locale={locale} />}
            {active.id === 'campaign-cover' && <CampaignCoverHero locale={locale} />}
          </motion.div>
        </AnimatePresence>
      </div>
    </AtmosphereSection>
...
function EditorialRoseHero({ locale }: { locale: Locale }) {
  return (
    <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <EditorialMedia src={resolveImageSrc('/products/luminous-glow-serum-1.jpg')} alt="Luminous serum campaign" className="aspect-[4/5] rounded-[2rem] shadow-editorial" />
        <EditorialMedia src={resolveImageSrc('/products/glass-skin-essence-1.jpg')} alt="Glass essence campaign" className="aspect-[4/5] rounded-[2rem] shadow-editorial sm:mt-10" />
      </div>
    </div>
  )
}

function ModernMinimalHero({ locale }: { locale: Locale }) {
  return (
    <div className="rounded-[2.2rem] border border-[#ecd8ce] bg-white/85 px-6 py-12 lg:px-14">
      ...
      <div className="mt-4 grid gap-10 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="rounded-3xl border border-rose-mauve/20 bg-gradient-to-b from-white to-[#fcf3ef] p-5">
          <EditorialMedia src={resolveImageSrc('/products/glass-skin-essence-1.jpg')} alt="Minimal hero product" className="aspect-[5/6] rounded-2xl" />
        </div>
      </div>
    </div>
  )
...
function ProductSpotlightHero({ locale }: { locale: Locale }) {
  return (
    <div className="relative overflow-hidden rounded-[2.5rem] border border-rose-mauve/20 bg-gradient-to-br from-[#fff7f3] via-[#faedf2] to-[#f7eee6] p-7 lg:p-10">
      ...
      <div className="relative grid items-center gap-8 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="rounded-[2rem] border border-white/70 bg-white/70 p-3 shadow-elevated">
          <EditorialMedia src={resolveImageSrc('/products/luminous-glow-serum-1.jpg')} alt="Spotlight product" className="aspect-[4/5] rounded-[1.5rem]" />
        </div>
        <div>
...
function SoftCampaignHero({ locale }: { locale: Locale }) {
  return (
    <div className="relative overflow-hidden rounded-[2.5rem] border border-rose-mauve/20 bg-white/70 p-8 backdrop-blur-sm lg:p-12">
      ...
      <div className="relative grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
        ...
        <div className="rounded-[2rem] bg-white/60 p-3 shadow-elevated">
          <EditorialMedia src={resolveImageSrc('/products/glass-skin-essence-1.jpg')} alt="Romantic campaign media" className="aspect-[4/5] rounded-[1.7rem]" />
        </div>
      </div>
    </div>
  )
...
function IvoryLabHero({ locale }: { locale: Locale }) {
  return (
    <div className="grid gap-7 lg:grid-cols-[1fr_1fr]">
      ...
      <div className="rounded-[2rem] border border-rose-mauve/20 bg-white/70 p-3">
        <EditorialMedia src={resolveImageSrc('/products/luminous-glow-serum-1.jpg')} alt="Science hero media" className="aspect-[6/5] rounded-[1.6rem]" />
      </div>
    </div>
  )
}

function FilmFrameHero({ locale }: { locale: Locale }) {
  return (
    <div className="overflow-hidden rounded-[2.4rem] border border-rose-mauve/25 bg-[#fff9f5] p-6 lg:p-8">
      <div className="rounded-[1.8rem] border border-[#e8d7c8] bg-gradient-to-br from-[#fef8f4] to-[#f7ece3] p-4">
        <div className="relative aspect-[16/9] overflow-hidden rounded-[1.3rem] border border-white/70 bg-charcoal/90">
          <EditorialMedia src={resolveImageSrc('/products/luminous-glow-serum-1.jpg')} alt="Video concept still" className="absolute inset-0 opacity-70" overlayClassName="bg-gradient-to-r from-charcoal/50 to-charcoal/20" />
...
function ImageLedHero({ locale }: { locale: Locale }) {
  return (
    <div className="relative overflow-hidden rounded-[2.5rem]">
      <EditorialMedia src={resolveImageSrc('/products/glass-skin-essence-1.jpg')} alt="Full background hero image" className="aspect-[16/9] min-h-[460px]" overlayClassName="bg-gradient-to-r from-charcoal/62 via-charcoal/30 to-charcoal/18" />
...
function CommerceSplitHero({ locale }: { locale: Locale }) {
  return (
    <div className="grid gap-0 overflow-hidden rounded-[2.3rem] border border-rose-mauve/20 bg-white lg:grid-cols-2">
      ...
      <div className="bg-gradient-to-br from-[#fbeef2] to-[#f7ede4] p-4 lg:p-6">
        <EditorialMedia src={resolveImageSrc('/products/luminous-glow-serum-1.jpg')} alt="Commerce split product" className="h-full min-h-[360px] rounded-[1.7rem]" />
      </div>
    </div>
  )
}
    ...
    <div className="relative overflow-hidden rounded-[2.5rem] border border-white/70 bg-[linear-gradient(135deg,#fffaf7_0%,#fdf2f6_50%,#f8f0e7_100%)] p-8 lg:p-12">
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_0.9fr] lg:items-center">
        ...
        <div className="rounded-[2rem] border border-white/80 bg-white/42 p-4 backdrop-blur-xl shadow-[0_20px_40px_rgba(203,168,150,0.22)]">
          <EditorialMedia src={resolveImageSrc('/products/glass-skin-essence-1.jpg')} alt="Mist glass hero media" className="aspect-[4/5] rounded-[1.5rem]" />
        </div>
      </div>
    </div>
  )
...
function CampaignCoverHero({ locale }: { locale: Locale }) {
  return (
    <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
      ...
      <div className="relative rounded-[2rem] border border-rose-mauve/20 bg-gradient-to-b from-[#faeef2] to-[#f7eee5] p-3">
        <EditorialMedia src={resolveImageSrc('/products/luminous-glow-serum-1.jpg')} alt="Campaign cover visual" className="aspect-[4/5] rounded-[1.6rem]" overlayClassName="bg-gradient-to-t from-charcoal/45 via-transparent to-transparent" />
