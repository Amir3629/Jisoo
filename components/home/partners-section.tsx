...
export function PartnersSection() {
  ...
  const copy = {
    ...
    lead: locale === 'ar' ? 'مستحضرات تمنحكِ' : locale === 'fr' ? 'Des cosmétiques qui inspirent' : locale === 'de' ? 'Kosmetik, die inspiriert' : locale === 'ko' ? '자신감을 전하는' : locale === 'tr' ? 'İlham veren kozmetikler' : '{copy.lead}',
    confidence: locale === 'ar' ? 'الثقة' : locale === 'fr' ? 'CONFIANCE' : locale === 'de' ? 'VERTRAUEN' : locale === 'ko' ? '자신감' : locale === 'tr' ? 'GÜVEN' : '{copy.confidence}',
